...
class FinerWeightedTarget:
    ...
    """
    def __init__(self, main_category, comparison_categories, alpha, truncate_weight):
        self.main_category = main_category
        self.comparison_categories = comparison_categories
        self.alpha = alpha
        self.truncate_weight = truncate_weight
        #print(f"[utils/model_targets.py] FinerWeightedTarget: alpha = {self.alpha}")
    ...
    def __call__(self, model_output):
        ...
        prob = torch.softmax(model_output, dim=-1) #NOTE: Aggregation (eq (8))

        weights = [prob[idx] if model_output.ndim == 1 else prob[..., idx] for idx in self.comparison_categories]
        
        if self.truncate_weight:
            wn = torch.where(wn > 0,wn,torch.zeros_like(wn))
            cmp_selected_weights =[]
            for idx in self.comparison_categories:
                _weight = select(idx)
                _weight = torch.where(_weight > 0, _weight, torch.zeros_like(_weight))
                cmp_selected_weights.append(_weight)
            numerator = sum(w * (wn - self.alpha * cw) for w, cw in zip(weights, cmp_selected_weights))                     
        else:
