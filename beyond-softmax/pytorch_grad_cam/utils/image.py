...
def show_cam_on_image_batch(img_batch: np.ndarray,
                            mask_batch: np.ndarray,
                            use_rgb: bool = False,
                            colormap: int = cv2.COLORMAP_JET,
                            image_weight: float = 0.5) -> np.ndarray:
    """ This function overlays the cam mask on each image in a batch as a heatmap.
    
    :param img_batch: Batch of base images in RGB or BGR format.
    :param mask_batch: Batch of cam masks.
    :param use_rgb: Whether to use an RGB or BGR heatmap, this should be set to True if 'img' is in RGB format.
    :param colormap: The OpenCV colormap to be used.
    :param image_weight: The final result is image_weight * img + (1-image_weight) * mask.
    :returns: Batch of images with the cam overlay.
    """
    batch_size = img_batch.shape[0]
    cam_batch = []

    if np.max(img_batch) > 1:
        raise Exception(
            "The input images should be in np.float32 format with the range [0, 1]")
    ...
    for i in range(batch_size):
        ...
        # Generate heatmap for each mask in the batch
        heatmap = cv2.applyColorMap(np.uint8(255 * mask), colormap)
        if use_rgb:
            heatmap = cv2.cvtColor(heatmap, cv2.COLOR_BGR2RGB)
        heatmap = np.float32(heatmap) / 255
        ...
        cam_batch.append(np.uint8(255 * cam))

    return np.stack(cam_batch, axis=0)
